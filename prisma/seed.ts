...
async function main() {
  ...
  const users = await Promise.all([
    prisma.user.create({ data: { username: 'Alice', password: hashed, display_name: 'Alice M.', img: 'https://www.neozone.org/blog/wp-content/uploads/2021/04/chat-espion-002.jpg' } }),
    prisma.user.create({ data: { username: 'Bob', password: hashed, display_name: 'Bob B.', img: 'https://ziggyfamily.com/cdn/shop/files/chat-espionne-sous-la-douche_1000x.png?v=1745423604' } }),
    prisma.user.create({ data: { username: 'Charlie', password: hashed, display_name: 'Charlie C.', img: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSRewheEYrkp1CREHY2zen3pFgDIwWSfIs9PQ&s' } }),
    prisma.user.create({ data: { username: 'Dave', password: hashed, display_name: 'Dave L.', img: 'https://www.filalapat.fr/sites/default/files/inline-images/AdobeStock_282605628%20%281%29.jpeg' } }),
  ]);
  const [u1, u2, u3, u4] = users;

  // 4. Thèmes JSON (Conformes à ton ThemeDto)
  const themeBlue = JSON.stringify({
    primary_color: '#0000FF', primary_color_dark: '#00008B', accent_color: '#ADD8E6', text_color: '#FFFFFF', accent_text_color: '#000000'
  });
  const themeRed = JSON.stringify({
    primary_color: '#FF0000', primary_color_dark: '#8B0000', accent_color: '#d3818f', text_color: '#FFFFFF', accent_text_color: '#000000'
  });

  // 5. Channel GÉNÉRAL (Tout le monde)
    ...
    data: {
      ...
      members: {
        create: [
          ...
          { userId: u2.id }, { userId: u3.id }, { userId: u4.id }
        ]
      }
    }
  });

  // 6. Channel PRIVÉ 1 (Alice & Bob uniquement)
  ...
  // 7. Channel PRIVÉ 2 (Charlie & Dave uniquement)
